...
export class Relay {
  ...
  handleFile(conn: Conn, msg: FileMsg): void {
    if (!conn.room || !conn.device) {
      this.send(conn, { t: 'error', code: 'not-joined', message: 'join before sending file' });
      return;
    }
    const room = this.rooms.get(conn.room);
    if (!room) return;

    // Same fan-out as clips/cmds/stats/notes/sms: forward verbatim to the *other* peer(s); never
    // echo. The file bytes are E2E-encrypted (nonce/ct), so the relay stays a verbatim, opaque pipe.
    const payload = JSON.stringify(msg);
    let delivered = 0;
    for (const peer of room.values()) {
      if (peer === conn || peer.ws.readyState !== WebSocket.OPEN) continue;
